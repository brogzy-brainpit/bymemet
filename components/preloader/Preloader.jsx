...
import PreloaderIntro from './PreloaderIntro'
...
const AnimateSvg = {
  ...
  exit: (i) => ({
    opacity:0,
    //  strokeWidth:['60%','50%','30%','10%', '0%'],
    // strokeLinecap:"round",
    // pathLength: 0, // ⬅️ reverse “cleaning” direction (end → start)
    transition: { delay:.6,duration: 1, ease: 'linear'}
    
  })
}
  return (
    <motion.div  className='z-preloader fixed top-0 left-0 bg-slate600 h-svh w-full'>
      <div className="relative w-full h-full">
        <PreloaderCounter/>
        {/* <div className='mx-auto absolute inset-0 z-[100] bgslate-500 flex items-center justify-center w-[80%] h-full'>
          <PreloaderIntro isLoading={isLoading}/>
          </div> */}

<motion.div  variants={AnimateSvg} 
initial='initial' exit='exit' className='absolute w-full bg-brand-white h-full'>
  {/* <svg xmlns="http://www.w3.org/2000/svg" height="100%" width="100%"  viewBox="0 0 3222 3114" fill="none" preserveAspectRatio="none" class="transition-scribble text-brand-background">
